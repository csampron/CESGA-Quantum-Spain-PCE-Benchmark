#!/usr/bin/env python3
# graph_bin_pct_feasible_and_best_perfect_all_k.py
...
def cargar_porcentajes(db_files):
    ...
    for k, db_name in db_files.items():
        ...
        try:
            ...
            FROM BIN_feasibility_counts
            """)
        except Exception as e:
            print(f"⚠️ Error leyendo BIN_feasibility_counts en {db_name}: {e}")
            conn.close()
            continue
...
def graficar_porcentaje(data_no, data_reg, metric_name, ylabel, filename_suffix):
    ...
    for opt in optimizers:
        ...
        plt.xlabel("Bin Packing instance")
        plt.ylabel(ylabel)
        ...
        plt.savefig(
            os.path.join(FIG_DIR, f"{opt}_{filename_suffix}.png"),
            dpi=200,
            bbox_inches="tight"
        )
