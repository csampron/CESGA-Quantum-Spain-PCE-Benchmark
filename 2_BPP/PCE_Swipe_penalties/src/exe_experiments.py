...
###
### Funciones para:
### - Generar combinaciones de experimentos de BPP
### - Filtrar combinaciones según criterios
### - Ejecutar un experimento individual de BPP
...
def ejecutar_experimentos(exp_list, optimizer_params, alpha, beta, lambda_1, lambda_2, lambda_3, maxiter, n_shots, nqpus, cunqa_str, family_name, output_dir):   
    """
    ...
    exp_list : list
        ...
        donde:
            - problema: nombre del problema (ej., "BPP")
    ...
    - Ejecuta BPP mediante `ejecutar_bpp()`, que:
        • prepara el circuito,
        • ejecuta el optimizador,
        • guarda outputs (csv, json, etc.),
        • devuelve metadatos sobre las rutas creadas.
    - Devuelve las rutas de salida generadas por ejecutar_bpp.
    ...
    from src.exe_bpp import ejecutar_bpp      # Función principal de ejecución de BPP
    from src.op_graph import load_bpp  # Carga de grafos y utilidades
    from pathlib import Path

    # === 1. Extraer parámetros del experimento ===
    problema = exp_list[0]     # Ejemplo: "BPP"
