...
def ejecutar_vrp(
    ...
    num_qubits,
    get_vrp_candidate_routes,
    build_vrp_solution_from_candidates,
    vrp_postprocess,
    refine_vrp_routes
    )
    ...
    if output_dir is not None:
        ...
        nombre_archivo_csv = f"VRP_{num_ver}_{instancia}_{optimizer}_{k}.csv"
        nombre_archivo_csv_iter =f"VRP_{num_ver}_{instancia}_{optimizer}_{k}_iter.csv"
    ...
    # lógica original existente (compatibilidad)
        ...
        # lógica original existente (compatibilidad)
        if cunqa_str_arg == "Simulation":
            subcarpeta = f"Resultados/VRP/k_{k}/{cunqa_str_arg}/{num_ver}_vertices/inst_{instancia}/{optimizer}"
        else:
            subcarpeta = f"Resultados/VRP/k_{k}/{cunqa_str_arg}/{nqpus}qpus/{num_ver}_vertices/inst_{instancia}/{optimizer}"
    ...
    # Después de run_vqe_optimization y seleccionar el mejor exp_map
    last_exp_map = best["exp_map"]

    # Obtener rutas candidatas como objetos tipo TSP
    candidate_routes, x_values = get_vrp_candidate_routes(
        last_exp_map, G, alpha, A=2
    )

    # 2. solución VRP
    initial_routes, initial_cost, initial_feasible = build_vrp_solution_from_candidates(
        candidate_routes, G, vehicle_capacity = Capacity, A=2
    )

    print("Initial routes:", initial_routes)
    print("Feasible:", initial_feasible)
    print("Initial cost:", initial_cost)

    # 3. postproceso SOLO si es factible
    if initial_feasible:
        clean_routes = vrp_postprocess(initial_routes, G)

        refined_routes, refined_cost = refine_vrp_routes(clean_routes, G)

        print("Refined routes:", refined_routes)
        print("Refined cost:", refined_cost)
    ...
    dic_resultado = {
        ...
        # =====================================================
        # CANDIDATE LEVEL (NO FACTIBILIDAD GLOBAL)
        # =====================================================

        "candidate_routes": candidate_routes,

        # =====================================================
        # INITIAL VRP SOLUTION
        # =====================================================
    ...
    if output_dir is not None:
        ...
        nombre_archivo = f"VRP_{num_ver}_{instancia}_{optimizer}_{k}.json"
    ...
    # === 10. Devolver resultado individual ===
