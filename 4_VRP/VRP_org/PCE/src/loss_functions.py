...
def loss_func_estimator(
    ...
    depot = 0  # asumimos nodo 0 como depósito
    
    m = n_nodes
    A = m
    node_exp_map = select_nodes_from_aux(aux, m**3, list_size, return_concatenated=True)

        # ------------------------------
    # 3. Variables relajadas x_{a,v,s} en [0,1]
    # ------------------------------
    ...
    # índice lineal: idx(a,v,s)
    def idx_avs(a, v, s):
        return a * (n_nodes * n_nodes) + v * n_nodes + s

    # ------------------------------
    # 4. Calcular H_A (coste de rutas)
    # ------------------------------
    H_A = 0.0
    for a in range(A):
    ...
    # 5. Penalizaciones
    # ------------------------------
    H_B = 0.0  # cada cliente visitado exactamente una vez
    for v in range(1, n_nodes):  # excluimos depósito
        visit_sum = sum(x_relaxed[idx_avs(a,v,s)] for a in range(A) for s in range(n_nodes))
        H_B += (1 - visit_sum)**2

    H_S = 0.0  # cada vehículo empieza en depósito
    for a in range(A):
        H_S += (1 - x_relaxed[idx_avs(a,depot,0)])**2

    H_E = 0.0  # cada vehículo termina en depósito
    for a in range(A):
        H_E += (1 - x_relaxed[idx_avs(a,depot,n_nodes-1)])**2
