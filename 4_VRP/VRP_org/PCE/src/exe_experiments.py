...
def ejecutar_experimentos(exp_list, optimizer_params, alpha, beta, maxiter, n_shots, nqpus, cunqa_str, family_name):   
    ...
    from src.exe_vrp import ejecutar_vrp       # Función principal de ejecución de MaxCut
    from src.op_graph import load_vrp_graph  # Carga de grafos y utilidades
    ...
    k          = exp_list[4]           # Parámetro adicional del modelo (ej., compresión)
    ...
    G, num_ver, depot, capacity, demand = load_vrp_graph(f"{parent}/graphs/{problema.lower()}_{tamaño}_{instancia}.vrp")
    ...
    # === 4. Ejecutar MaxCut ===
    dic_resultado, subcarpeta, ruta_csv, ruta_csv_iter = ejecutar_vrp(
        G=G,
        Capacity=capacity,
        optimizer=optimizer,
