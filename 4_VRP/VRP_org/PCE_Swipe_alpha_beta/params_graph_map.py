import matplotlib.pyplot as plt
from pathlib import Path
import pandas as pd
import json
import numpy as np
from datetime import datetime

# ====================================================
# 1) LOAD RESULTS (VRP REAL)
# ====================================================
def load_vrp_results_from_path(path):
    base_path = Path(path)

    if not base_path.exists():
        raise FileNotFoundError(f"❌ No existe el directorio: {base_path}")

    print(f"📂 Leyendo resultados VRP en: {base_path}")

    records = []

    for k_folder in base_path.iterdir():
        if not k_folder.is_dir() or not k_folder.name.startswith("k_"):
            continue

        # soporta k numérico o string
        k_raw = k_folder.name.split("_", 1)[1]
        try:
            k_val = int(k_raw)
        except:
            k_val = k_raw

        print(f"\n📁 Procesando {k_folder.name} (k={k_val})")

        for folder in k_folder.iterdir():
            if not folder.is_dir() or not folder.name.lower().startswith("alpha_"):
                continue

            try:
                parts = folder.name.split("_")
                alpha = float(parts[1])
                beta  = float(parts[3])

                resultados_dir = folder / "Resultados"

                # fallback si no existe
                if not resultados_dir.exists():
                    resultados_dir = folder

                json_files = list(resultados_dir.glob("*.json")) + list(resultados_dir.glob("*.JSON"))

                if not json_files:
                    continue

                values = []

                for jf in json_files:
                    with open(jf, "r") as f:
                        data = json.load(f)

                        results = data.get("resultados", [])
                        if not results:
                            results = [data]

                        for res in results:
                            # 🔥 CAMBIO CLAVE
                            val = res.get("refined_cost")

                            if val is None:
                                val = res.get("initial_cost")

                            if val is not None:
                                values.append(val)

                if not values:
                    print(f"⚠️ Sin valores válidos en {folder}")
                    continue

                mean_val = np.mean(values)
                std_val  = np.std(values)
                min_val  = np.min(values)

                records.append([k_val, alpha, beta, mean_val, std_val, min_val])

            except Exception as e:
                print(f"⚠️ Ignorado {folder.name}: {e}")

    if not records:
        raise RuntimeError("⚠️ No se encontraron resultados válidos.")

    df = pd.DataFrame(
        records,
        columns=["k", "alpha", "beta", "mean", "std", "min_cost"]
    )

    return df


# ====================================================
# 2) HEATMAP
# ====================================================
def plot_heatmap(arr, x_vals, y_vals, title, xlabel, ylabel, cmap, out_path):
    fig, ax = plt.subplots(figsize=(12, 6))
    c = ax.imshow(arr, origin="lower", aspect="auto", cmap=cmap)

    ax.set_xticks(np.arange(len(x_vals)))
    ax.set_xticklabels([f"{x:.2f}" for x in x_vals])

    ax.set_yticks(np.arange(len(y_vals)))
    ax.set_yticklabels([f"{y:.2f}" for y in y_vals])

    ax.set_xlabel(xlabel)
    ax.set_ylabel(ylabel)
    ax.set_title(title)

    fig.colorbar(c, ax=ax)
    fig.savefig(out_path, dpi=300, bbox_inches="tight")
    plt.close(fig)


# ====================================================
# 3) SAVE SUMMARY
# ====================================================
def save_best_summary(path, min_val, best_rows, alpha_vals, beta_vals):
    path = Path(path)

    summary = {
        "timestamp": datetime.now().isoformat(timespec="seconds"),
        "best_cost": float(min_val),
        "num_best_combinations": int(len(best_rows)),
        "best_combinations": [
            {"alpha": float(row["alpha"]), "beta": float(row["beta"])}
            for _, row in best_rows.iterrows()
        ],
        "alpha_unique": int(len(alpha_vals)),
        "beta_unique": int(len(beta_vals)),
    }

    json_path = path / "best_summary.json"
    with open(json_path, "w") as f:
        json.dump(summary, f, indent=2)

    txt_path = path / "best_summary.txt"
    with open(txt_path, "w") as f:
        f.write("🎯 Mejor coste\n")
        f.write(f"Coste mínimo: {min_val}\n\n")
        f.write("Combinaciones alpha/beta:\n")
        for row in summary["best_combinations"]:
            f.write(f"  alpha={row['alpha']}, beta={row['beta']}\n")

    print(f"💾 Resumen guardado en {path}")


# ====================================================
# 4) MAIN
# ====================================================
def plot_vrp_results(base_path):
    base_path = Path(base_path)
    df = load_vrp_results_from_path(base_path)

    for k_val in sorted(df["k"].unique(), key=lambda x: str(x)):
        print(f"\n📊 Procesando k = {k_val}")

        df_k = df[df["k"] == k_val]

        min_val = df_k["min_cost"].min()
        best_rows = df_k[df_k["min_cost"] == min_val]

        print(f"🎯 Mejor coste: {min_val}")
        for _, row in best_rows.iterrows():
            print(f"   alpha={row['alpha']}, beta={row['beta']}")

        pivot_mean = df_k.pivot(index="beta", columns="alpha", values="mean")
        pivot_std  = df_k.pivot(index="beta", columns="alpha", values="std")

        arr_mean = pivot_mean.to_numpy()
        arr_std  = pivot_std.to_numpy()

        beta_vals = pivot_mean.index.to_numpy()
        alpha_vals = pivot_mean.columns.to_numpy()

        out_dir = base_path / f"k_{k_val}"
        out_dir.mkdir(exist_ok=True)

        plot_heatmap(
            arr_mean,
            alpha_vals,
            beta_vals,
            title=f"Mean refined cost (k={k_val})",
            xlabel="Alpha",
            ylabel="Beta",
            cmap="viridis",
            out_path=out_dir / "cost_mean.png",
        )

        plot_heatmap(
            arr_std,
            alpha_vals,
            beta_vals,
            title=f"Std refined cost (k={k_val})",
            xlabel="Alpha",
            ylabel="Beta",
            cmap="plasma",
            out_path=out_dir / "cost_std.png",
        )

        save_best_summary(out_dir, min_val, best_rows, alpha_vals, beta_vals)

    print("\n✅ Heatmaps VRP generados correctamente.")


# ====================================================
# 5) RUN
# ====================================================
if __name__ == "__main__":
    base_path = "/mnt/netapp1/Store_CESGA/home/cesga/falonso/z_VRP/VRP_org/PCE_barrido_alpha_beta/Experimentos/VRP_m_5_inst_1/31_03_2026_10_50"
    plot_vrp_results(base_path)