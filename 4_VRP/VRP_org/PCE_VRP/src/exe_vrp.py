...
def ejecutar_vrp(
    ...
    if cunqa_str_arg == "Simulation":
        subcarpeta = f"Resultados/VRP/k_{k}/{cunqa_str_arg}/{num_ver}_vertices/{optimizer}"
    else:
        subcarpeta = f"Resultados/VRP/k_{k}/{cunqa_str_arg}/{nqpus}qpus/{num_ver}_vertices/{optimizer}"


    # Definir rutas para guardar resultados (CSV)
    nombre_archivo_csv = f"VRP_{num_ver}_{optimizer}_{k}.csv"
    nombre_archivo_csv_iter =f"VRP_{num_ver}_{optimizer}_{k}_iter.csv"
    ...
    nombre_archivo = f"VRP_{num_ver}_{optimizer}_{k}.json"
