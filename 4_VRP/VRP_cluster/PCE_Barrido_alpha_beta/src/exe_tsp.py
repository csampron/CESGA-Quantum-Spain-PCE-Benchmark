...
def ejecutar_tsp(
    ...
    cunqa_str_arg,
    family_name,
    tamaño,     # nuevo argumento
    instancia,  # nuevo argumento
    idx,        # nuevo argumento: índice del cluster   
    output_dir
):
    """
    ...
    # --- Crear subcarpeta para este cluster ---
    ...
    if output_dir is not None:
        ...
        subcarpeta = os.path.join(output_dir, f"Resultados/cluster_{idx}")
        if not os.path.isdir(subcarpeta):
            os.makedirs(subcarpeta, exist_ok=True)
        
        nombre_archivo_csv = f"TSP_{num_ver}_{optimizer}_{k}.csv"
        nombre_archivo_csv_iter = f"TSP_{num_ver}_{optimizer}_{k}_iter.csv"
    ...
    # lógica original existente (compatibilidad)
        ...
        if cunqa_str_arg == "Simulation":
            subcarpeta = f"Resultados/VRP/{cunqa_str_arg}/{tamaño}_vertices/inst_{instancia}/{optimizer}/cluster_{idx}"
        else:
            subcarpeta = f"Resultados/VRP/{cunqa_str_arg}/{nqpus}qpus/{tamaño}_vertices/inst_{instancia}/{optimizer}/cluster_{idx}"
    ...
    if output_dir is not None:
        ...
        nombre_archivo = f"TSP_{num_ver}_{optimizer}_{k}.json"
