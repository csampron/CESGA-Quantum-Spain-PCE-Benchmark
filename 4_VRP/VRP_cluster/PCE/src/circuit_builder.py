...
import numpy as np
from qiskit.circuit.library import RXGate, RYGate, RZGate, RXXGate, RYYGate, RZZGate, U3Gate, CXGate, CZGate, HGate, SdgGate
...
class Circuit(object):
    """
    Clase que define un circuito cuántico paramétrico (ansatz) configurable.

    Parámetros
    ----------
    size : int, opcional (por defecto = 6)
        Número de qubits del circuito. Define el ancho del circuito.

    p : int, opcional (por defecto = 0)
        Número de capas (repeticiones) del ansatz. Cada capa suele incluir 
        una parte de rotaciones locales y otra de entrelazamiento.

    entanglement : str, opcional (por defecto = "CNOT")
        Tipo de puerta de entrelazamiento usada entre qubits.
        Ejemplos: "CNOT", "CZ", "RXX", "RYY", "RZZ", "SU4", "Taylor_efficient".

    rotation : str, opcional (por defecto = "U3")
        Tipo de puerta de rotación aplicada a cada qubit.
        Ejemplos: "RX", "RY", "RZ", "U3", "Taylor_efficient".

    connectivity : str, opcional (por defecto = "alternating_closed")
        Topología de conectividad entre qubits. Determina qué qubits se
        entrelazan en cada capa. 
        Ejemplos: "ladder_open", "ladder_closed", "brickwork_single_open",
        "brickwork_double", "round_robin", "2D_lattice", "random", "d-regular".

    initial_param : list, array o None, opcional
        Valores iniciales de los parámetros del circuito. Si no se proporciona,
        se usarán parámetros simbólicos (ParameterVector).

    Atributos
    ---------
    entang_list : list
        Lista de pares de qubits que define las conexiones de entrelazamiento 
        generadas por `_define_connectivity_()`.

    circuit_representation : QuantumCircuit o None
        Objeto QuantumCircuit de Qiskit que representa el circuito compilado.
        Se inicializa con None y se genera tras ejecutar `compile_circuit()`.
    """
