...
def plot_error_vs_shots(size, k, error_type="mse", base_folder="Resultados"):
    """
...
def generar_graficas_varios_m(lista_m, k, base_folder="Resultados"):
    """
...
def plot_errors_vs_m_separado(lista_m, k, base_folder="Resultados"):
    """
    ...
    for etype in errores:
        if etype != "max_error":
            ...
            filename = os.path.join(output_folder, f"noisy_{etype}_vs_m_k_{k}.png")
            plt.savefig(filename, dpi=300)
            plt.close()
            print(f"[OK] Guardado {filename}")
        else:
...
k = 4
direc = "/mnt/netapp1/Store_CESGA/home/cesga/falonso/PCE_SHOTS/Noisy/MaxCut/PCE_Shots_Estimation/Resultados"
